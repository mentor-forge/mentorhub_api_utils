...
- Verified `api_utils/services/__init__.py` already imports and lists `ExternalEventService` and `NotificationService` in `__all__` (R070/R071). No edit needed.
- Bumped `pyproject.toml` `version` `0.6.0` → `0.7.0` (minor: additive Config constants + two new services; dropped unused collection keys).
- `README.md`: pin example `api-utils==0.7.0`; listed ExternalEvent/Notification in `services/` and the shared surface; one paragraph on inline Config constants (F-UA08). Left infinite-scroll deprecation note in place.
- `pipenv run test`: **286 passed**, 24 deselected. `pipenv run lint`: failed on pre-existing files (not this task). `black --check api_utils/services/__init__.py`: clean. `pipenv run build`: `api_utils-0.7.0.tar.gz` and `api_utils-0.7.0-py3-none-any.whl`.
- Orchestrator confirmation: `pipenv run test` 286 passed, build `api_utils-0.7.0`. Status set to Shipped.
