# Migrate Customer API list endpoints off infinite-scroll cursors

> **Cross-repo issue artifact.** Paste-ready description for follow-on planning
> in **`mentorhub_customer_api`**. Not part of the current `PENDING.*`
...
deprecated cursor contract. Nine local list services import
...
`Pipfile` currently pins `api-utils==0.2.1` (predates `list_query`). Bump the
pin, switch every list endpoint to the Standardized Get List pattern used by
`mentorhub_mentee_api` (`parse_list_request` + `execute_list_query`), then
delete all infinite-scroll imports so api_utils can remove the module.
...
Same nine services and routes plus `docs/openapi.yaml` (nine list operations
and a shared cursor schema). Unit/route tests assert the envelope; E2E tests
...
### Replace list contract (every list endpoint)
...
Declare a `*_LIST_FILTERS` / `*_LIST_ORDER` next to each service (name
