...
class TestConfigDefaults(unittest.TestCase):
    ...
    def test_default_string_secret_properties(self):
        for key, default in self.config.config_string_secrets.items():
            if key == "JWT_SECRET":
                ...
                self.assertEqual(
                    getattr(self.config, key), "test-secret-for-defaults-testing"
                )
            else:
                self.assertEqual(getattr(self.config, key), default)
    ...
    def test_default_string_ci(self):
        for key, default in {
            **self.config.config_strings,
            **self.config.config_ints,
        }.items():
            self._test_config_default_value(key, default)

    def test_default_secret_ci(self):
        for key, default in {
            **self.config.config_string_secrets,
            **self.config.config_json_secrets,
        }.items():
            if key == "JWT_SECRET":
